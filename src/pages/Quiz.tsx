...
export function Quiz() {
  const { activeLessonId, setView, completeLesson, updateStreak, customLessons } = useStore();
  ...
  const generateOptions = () => {
    ...
    // Shuffle and pick 3
    const shuffledAll = allVi.sort(() => 0.5 - Math.random());
  ...
  const handleNext = () => {
    if (currentIndex < lesson.words.length - 1) {
      ...
      completeLesson(lesson.id, 15); // +15 XP
      updateStreak();
      setCurrentIndex(c => c + 1);
    }
  };
  ...
  if (isFinished) {
    return (
      <div className="flex flex-col h-[100dvh] bg-emerald-50 p-6">
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          <div className="w-32 h-32 bg-emerald-400 rounded-full flex items-center justify-center mb-8 border-4 border-slate-900 shadow-[8px_8px_0px_0px_rgba(15,23,42,1)] animate-bounce">
             <Check size={80} className="text-slate-900 relative top-1" strokeWidth={4} />
          </div>
          <h2 className="text-4xl font-black text-slate-900 mb-4">Tuyệt vời!</h2>
          <p className="text-slate-900 font-bold mb-8 text-lg bg-emerald-100 border-2 border-slate-900 px-6 py-3 rounded-2xl shadow-[4px_4px_0px_0px_rgba(15,23,42,1)]">
            Bạn đã hoàn thành bài học và được cộng thêm 15 XP!
          </p>
        </div>
        <div className="space-y-4">
          <Button className="w-full" size="lg" onClick={() => setView('home')}>Tiếp tục học</Button>
        </div>
      </div>
    );
  }
      ...
      <div className="flex-1 flex flex-col p-6 max-w-2xl mx-auto w-full">
        <div className="hidden md:flex justify-end mb-4">
             <div className="bg-amber-100 border-2 border-slate-900 px-4 py-2 rounded-xl text-amber-900 font-black text-xs uppercase tracking-widest flex items-center gap-2">
                <Info size={16} /> Nhấn các phím số 1, 2, 3, 4 để chọn nhanh
             </div>
        </div>
      ...
      <div className={`p-6 border-t-4 border-slate-900 sm:px-8 mt-auto ${isChecking ? (isCorrect ? 'bg-emerald-50' : 'bg-rose-50') : 'bg-white'}`}>
         {isChecking && (
           <div className="mb-4 font-black flex items-center text-xl">
             {isCorrect ? (
               <span className="text-emerald-600 flex items-center"><span className="w-8 h-8 rounded-full bg-emerald-400 text-slate-900 border-2 border-slate-900 flex items-center justify-center mr-3 shadow-[2px_2px_0px_0px_rgba(15,23,42,1)]"><Check size={20} className="relative top-[1px]" strokeWidth={4} /></span> Tuyệt vời!</span>
             ) : (
               <span className="text-rose-600 flex items-center"><X size={28} className="mr-2" strokeWidth={4} /> Kết quả đúng là: <span className="ml-2 text-rose-800 bg-white border-2 border-slate-900 px-3 py-1 rounded-xl shadow-[2px_2px_0px_0px_rgba(15,23,42,1)]">{word.vi}</span></span>
             )}
           </div>
         )}
